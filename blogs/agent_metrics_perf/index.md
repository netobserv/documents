...
Memory decreased again, though not as much as before. There is a slight CPU increase which I attribute to the increased number of flows (this as well decreases after 10 minutes). So it sounds like despite approaching the utilization threshold, we're still on a positive trend, but it weakens.
...
Well, the situation has worsened, but not as much as I feared. The CPU has grown by +40% but the number of flows, while also growing, is not exploding. Memory consumption looks unchanged, although it has probably been re-balanced internally between the hashmap and other parts. As expected, the ring buffer is now consistently used, but still in decent proportions. Things can really get out of hand when the ratio exceeds ~0.05, but here it is still an order of magnitude below.
